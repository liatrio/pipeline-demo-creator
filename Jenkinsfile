    ...
    stage('Create Slack Channel') {
      steps {
        withCredentials([string(credentialsId: 'liatrio-demo-slack', variable: 'token')]){
          sh "curl -X POST -v -u ${env.token} https://liatrio-demo.slack.com/api/channels.create -H \"Content-Type: application/json\" -d \'{\"name\": \"\'${params.pipeline_name}\'\", \"token\": \"\'${env.token}\'\"}\'"
        }
      }
    }
  }
